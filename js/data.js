...
Prioritise gut safety AND liver safety. Prefer well-evidenced, well-tolerated, liver-safe options (creatine, whey isolate, omega-3, vitamin D). NEVER put anything with hepatotoxicity risk in "missing" — this includes, at ANY dose: curcumin/turmeric EXTRACT supplements, green-tea extract/EGCG, ashwagandha, niacin (nicotinic acid), vitamin A/retinol, iron (unless a doctor prescribes it), kava, herbal "detox"/fat-burner/pre-workout blends, and anabolic/prohormone/SARMs. Note: this bans the concentrated SUPPLEMENT/extract, not a normal culinary spice amount used in cooking. If the user's stack already contains such an item, put it in "remove" with the hepatic reason. Do not suggest changing prescribed medication — defer that to his doctor.`;
}
