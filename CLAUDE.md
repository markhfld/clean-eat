...
- **State** is browser-only (`js/store.js` → `localStorage`): API key, profile, UC mode, scans, supplements, labs. Nothing leaves the device except explicit Claude requests — or, if the user opts into **cloud sync**, an encrypted bundle to their gist.
- **Optional cross-device sync** (`js/sync.js`): serverless sync via a **private GitHub gist**. `store` exposes an `onChange` hook + `exportBundle`/`importBundle`; a change debounce-pushes an AES-GCM-encrypted (PBKDF2 passphrase) bundle to the gist, and launch/`Sync now` pulls (last-write-wins by `updatedAt`). Sync config (token/gistId/passphrase) is local-only and never itself synced. `SYNCED` keys in `store.js` define what travels. Requires a classic PAT with only `gist` scope.
